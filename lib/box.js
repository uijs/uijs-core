...
var box = module.exports = function(options) {
  ...
  obj.draw = function(parent_ctx) {
    var self = this;

    var new_state = JSON.stringify(obj.state());

    if (self._last_state && self._last_state === new_state && self._buffer) {
    ...
    var buffer = self._buffer = document.createElement('canvas');
