...
var box = module.exports = function(options) {
    ...
    if (bound_to_function){
      ...
      if(this._boundToFunction.length === 1){
        if(this._childrenCache.length > 0){
          ...
          this.pre_draw = pre_draw_no_children;
        }
      }
    }
    else{
      ...
      if (prop_index === -1) {
        ...
        if(this._boundToFunction.length === 0){
          if(this._childrenCache.length > 0){
            this.pre_draw = pre_draw_no_bound_to_functions;
          }
          else{
            this.pre_draw = empty_pre_draw;
          }
        }
      }
    }
  ...
  function first_pre_draw(ctx) {
    ...
    if(self.pre_draw === first_pre_draw) {alert('No pre_draw was calculated');};
  ...
  //var buffer_image_data = null;
  //var buffer_image = null;
  //var buffer_image_loaded = false;
    ...
    if(this._recalculate) {
      ...
      if(cache_use_counter < childrenlength) { // create the buffer in stages
        ...
        if(cache_use_counter === childrenlength){
          buffer_image_data = bufferCanvas.toDataURL('image/ jpeg', 0.2);
          buffer_image = new Image();
          buffer_image.onload = function(){
            buffer_image_loaded = true;
            console.log('buffer image loaded');
          }
          buffer_image.src = buffer_image_data;
        }
        */
        if(cache_use_counter === childrenlength){
        ...
        ctx.scale(oppositeScale, oppositeScale);
        ctx.drawImage(bufferCanvas, (x + xCache) * scale, (y + yCache) * scale); 
        //if(buffer_image_loaded){
        //  ctx.drawImage(buffer_image, (x + xCache) * scale, (y + yCache) * scale); 
        //}
      }
      cache_use_counter++;
    }
  ...
  obj.generate_buffer = generate_buffer;
