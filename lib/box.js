...
var box = module.exports = function(options) {
  var obj = defaults(options, {
    ...
    alpha: null,
    debug: false,
    interaction: constant(true), // send interaction events on this box. must be set to true for events to be emitted
    autopropagate: constant(true), // propagate interaction events to child boxes. if false, the parent needs to call `e.propagate()` on the event
  ...
  obj.draw = function(ctx) {
    var self = this;
    if (!self.visible() || (self.alpha && self.alpha() == 0)) return;
