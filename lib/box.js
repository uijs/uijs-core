...
var box = module.exports = function(options) {
  ...
  function resize_buffer_canvas(){
    bufferCanvas.width = widthCache;
    bufferCanvas.height = heightCache;
    //bufferCanvas.width = widthCache * window.devicePixelRatio;
    //bufferCanvas.height = heightCache * window.devicePixelRatio;
    bufferCanvas.width = widthCache * 2;
    bufferCanvas.height = heightCache * 2;
    bufferCanvas.style.width = (widthCache ? widthCache.toString() : '0');
    bufferCanvas.style.height = (heightCache ? heightCache.toString() : '0');
    //bufferCanvas.style.width = (widthCache ? widthCache.toString() : '0');
    //bufferCanvas.style.height = (heightCache ? heightCache.toString() : '0');
  ...
  function resize_buffer_canvas_no_scale(){
    if(useGivenBuffer){
      return;
    }
  ...
  obj.permiateOriginalCanvas = function(canvas, context, levels){
    ...
    if(levels > 0){
      ...
      this.children[0].permiateOriginalCanvas(canvas, context, levels);
    }
  }
