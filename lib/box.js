...
var box = module.exports = function(options) {
  var attributes = defaults(options, {
    ...
    recalculate: true, // <-- TODO: change to private
    redraw: true, // <-- TODO: change to private
    invalidatingVars: [], // <-- TODO: merge
  });
  ...
  //must have defaut value of true for controls which do not have buffer and we dont want them
  // to invalidate the entire canvas if nothing changes
  var finishedCacheGeneration = true; 
  var num_cache_uses = -1;
  var finishedSelfCacheGeneration = false;
  // TODO: unify the positioningchanged, and finishedCacheGeneration to one bool which signals whether the parent layer may use the cache
  var canvasNeedsResizing = false;
  var positioningChanged = false;
  var dimentionsChanged = false;
  var boundToFunction = {};
  ...
  function first_pre_draw(ctx) {
    ...
    var vars = self.invalidatingVars;
  ...
  function pre_draw(){
    ...
    var cuc = !(this.recalculate || dimentionsChanged);
    for (var i = 0; i < childrenlength; i++) {
      cuc &= children[i].pre_draw();
    };

    canUseCache = cuc;
    if(!canUseCache){
      num_cache_uses = -1;
      finishedCacheGeneration = false;
      finishedSelfCacheGeneration = false;
    }
    else if(!this.useBuffer){
      finishedCacheGeneration = true; 
    }
    ...
    return (cuc && !positioningChanged && finishedCacheGeneration) ? true : false;
  };

  function post_draw(){
    this.recalculate = false;
    positioningChanged = false;
    dimentionsChanged = false;
    if(canUseCache){
      num_cache_uses++;
    }
    else{
  ...
  function simple_draw(ctx, scale) {
    if(this.useBuffer && canUseCache){
      if(num_cache_uses === -1){
      ...
      else if(num_cache_uses === 0){ // set context on the buffer canvas and set context and draw on the canvas
        this.onSetContext(bufferContext);
        ctx.translate(xCache, yCache);
        this.onSetContext(ctx);
        onDrawFunctionCache.call(this, ctx);
      }
      else if(num_cache_uses === 1){ // draw on the buffer canvas and then draw the buffer canvas on the canvas
        onDrawFunctionCache.call(this, bufferContext);
        var opositeScale = 1/scale;
        ctx.scale(opositeScale, opositeScale);
  ...
  function draw_simple_container(ctx, scale) {
    // TODO: seperate to 2 functions and remove the if
    var children = childrenCache;
    var childrenlength = children.length;
    // TODO: Calculate useBuffer for simple containers automatically
    if(this.useBuffer && canUseCache){
      if(num_cache_uses < childrenlength) { // create the buffer in stages
        if(num_cache_uses === -1)
        {
          bufferContext.clearRect(0, 0, widthCache, heightCache); // clear buffer on the buffer canvas
        }
        else{
          children[num_cache_uses].draw(bufferContext, scale); // draw only one child on the buffer canvas (will later all children on the canvas)
        }
        // draw all children on the canvas since buffer is not yet ready
        ctx.translate(xCache, yCache);
        for (var i = 0; i < childrenlength; i++) {
          children[i].draw(ctx, scale);
        }
      }
  ...
  function draw_simple_container_with_self_drawing(ctx, scale) {
    var self = this;
    if(this.useBuffer && canUseCache) {
      if(!finishedCacheGeneration){
        if (!finishedSelfCacheGeneration){
          simple_draw.call(self, ctx, scale);
          canUseCache = false;
          draw_simple_container.call(self, ctx, scale);
          canUseCache = true;
          if(finishedCacheGeneration){
            finishedCacheGeneration = false;
            finishedSelfCacheGeneration = true;
            //the simple container will start at 0 (because of the increase 
            // in post_draw) and skip the clear buffer stage
            num_cache_uses = -1; 
          }
        }
        else{
          var tmp = num_cache_uses;
          num_cache_uses = 1; // to make the simple draw do the drawing stage
          simple_draw.call(self, ctx, scale);
          num_cache_uses = tmp;
          draw_simple_container.call(self, ctx, scale);
        }
        ...
        draw_simple_container.call(self, ctx, scale); //will use the cache
      }
    }
    else{
      finishedSelfCacheGeneration = false;
  ...
  function draw_optimized(ctx, scale) {
    var self = this;
  ...
  function determine_draw_function_and_execute(ctx, scale){
    var self = this;
