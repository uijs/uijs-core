...
var box = module.exports = function(options) {
  ...
  var firstTimeChildrenCbIsCalled = true;
  obj.watch('children', function(c, bound) {
    // the reason for this oddity is explained in the remarks below
    // TODO: think of a better way to do this (maybe even change the bind and watch API behaviors)
    //       because it is very unintuitive
    if  (firstTimeChildrenCbIsCalled) { 
      bound = !bound; 
      firstTimeChildrenCbIsCalled = false; 
    }
    if (!bound) {
      // if we are here then the setter of the property was called with a non function value 
      // or this is the cb which is called from inside the call to watch and this call binded a
      // constant value to the propery
      var _push = c.push;
      ...
      // if we are here then bind was called on children with a function 
      // or this is the cb which is called from inside the call to watch and this call
      // and this call didnt bind the property to a constant value because it was already
