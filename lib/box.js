...
var box = module.exports = function(options) {
  ...
  function pre_draw(){
    ...
    // to use its cache in case the positioning of this box has changed
    return (cuc && !positioningChanged) ? true : false;
  };
  ...
  function simple_draw(ctx, scale) {
    if(this.useBuffer && canUseCache){
      ...
      else if(num_cache_uses === 1){ // set context on the buffer canvas and set context and draw on the canvas
        this.onSetContext(bufferContext);
        ctx.translate(xCache, yCache);
        this.onSetContext(ctx);
        onDrawFunctionCache.call(this, ctx);
      }
      else if(num_cache_uses === 2){ // draw on the buffer canvas and then draw the buffer canvas on the canvas
  ...
  function draw_simple_container(ctx, scale) {
    ...
    if(this.useBuffer && canUseCache){
      ...
      for (var i = 0; i < childrenlength; i++) {
        children[i].draw(ctxToUse, scale);
      }
    }
    if(this.useBuffer){
      var opositeScale = 1/scale;
