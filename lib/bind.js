...
function _magic(prop) {
  ...
  var timestamps = self._timestamps = self._timestamps || {};
  var changed = self._changed = self._changed || {};
  ...
  if (!watched && bound) {
    Object.defineProperty(self, prop, {
      get: function() {
        ...
        if (timestamps[prop] !== ticks) {
          timestamps[prop] = ticks;
          prev = values[prop] = bound.call(self);
        }
        return prev;
      },
      set: function(curr) {
...
function changed() {
  var self = this;
  return self._changed ? self._changed : {};
}
...
module.exports = function(obj_or_fn) {
  ...
  if (typeof obj_or_fn === 'function') {
    ...
    obj.changed = obj.changed || changed;
