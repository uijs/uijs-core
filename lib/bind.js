...
function addWatch(obj){
  ...
  obj.watch = function(prop, cb) {
    ...
    //       Now, they must get the cb with bounded === true, even if no one explicitely called bind with a function, but
    //       also if the watch added the bind and it binded it to a constant value (a function which return a constant value,
    //       but still, there is no one who can change the result of the function without using the setter) - issue #37
    cb.call(obj, curr, newlyBounded);
    return this.$_bindings.ee.on(prop, cb);
  };

  obj.unwatch = function(prop, cb) {
    return this.$_bindings.ee.off(prop, cb);
  };
}
...
function bind(obj, prop, getter, emit) {
  ...
  function setter(newval) {
    ...
    if (newval && newval.$bind) {  
      bind(obj, prop, newval.$bind.fn, newval.$bind.emit);
      return newval;
    }
