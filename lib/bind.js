...
function bind(prop, fn) {
  var self = this;
  if (typeof fn === 'object' && fn.$bind) fn = fn.$bind; // support $bind results
  if (typeof fn !== 'function') throw new Error('`fn` must be a function');

  var property = _property.call(self, prop);
  var newlyBound = !property.bound;
  property.bound = fn;
...
// set multiple attributes (the non-animated version of `animate` ;-)
function set(values) {
  var self = this;
  ...
  values && Object.keys(values).forEach(function(key) {
    ...
    if (value && value.$bind) {
      self.bind.call(self, key, value.$bind);
    }
    else if (self !== values) { // no need to change anything if this is the same object
      self[key] = value;
    }
