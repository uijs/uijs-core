...
box.root = function() {
  var self = this;
  if (!self.parent || self.parent === self || !self.parent.root) return self;
...
box.tofront = function() {
  var self = this;
  if (!self.parent || self.parent === self) throw new Error('`tofront` requires that the box will have a parent');
...
box.siblings = function() {
  var self = this;
  if (!self.parent || self.parent === self) return [ self ]; // detached, no siblings but self
  return self.parent.all();
};

box.prev = function() {
  var self = this;
  if (!self.parent || self.parent === self) throw new Error('box must be associated with a parent')
...
box.remove = function(child) {
  ...
  if (!child) {
    if (!self.parent || self.parent === self) throw new Error('`remove()` will only work if you have a parent');
...
box.rest = function(child) {
  var self = this;
  if (!child) {
    if (!self.parent || self.parent === self) throw new Error('cannot call `rest()` without a parent');
