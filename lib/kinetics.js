...
function calculateSpeed(v0, acceleration, friction, delta_ts){
  ...
  var result = v0 * friction + delta_speed;
  return result;
}
  ...
  var returnValue = {
    ...
    animate: function(){
      ...
      for (var i = 0; i < timespan; i += time_unit) {
        self.velocity = calculateSpeed(self.velocity, acceleration(), friction(self.velocity), time_unit);
...
function carouselAnimation(carouselleftBase, carouselRightBase, initialPosition, initialVelocity, inSpringMode, initialSpringBase, options){
  ...
  var options = {
    ...
    friction: function(velocity){  
      determineSpring();
      if (swf.spring) {
        return springFriction(velocity);
      }
      else{
        return regularFriction(velocity);
      }
    }
  };
